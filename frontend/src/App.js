...
const AuthProvider = ({ children }) => {
  ...
  // Limpiar datos de usuarios no registrados al cerrar pestaña/navegador
  useEffect(() => {
    const handleBeforeUnload = () => {
      // Si no hay usuario autenticado, limpiar todos los datos locales
      if (!token || !user) {
        // Limpiar localStorage excepto las credenciales de demo si existen
        const keysToRemove = [];
        for (let i = 0; i < localStorage.length; i++) {
          const key = localStorage.key(i);
          // No eliminar el token si existe (usuario logueado)
          if (key && key !== 'token') {
  ...
  // Create axios instance with interceptor for token expiration
...
const Layout = ({ children }) => {
  ...
  // Swipe navigation DESHABILITADO por petición del usuario
  // El deslizamiento de páginas ha sido eliminado

  return (
    <div className="min-h-screen bg-background">

      {/* Header - Solo mostrar si el usuario está autenticado */}
      {user && (
        <header 
          className={`sticky top-0 z-40 border-b bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 transition-transform duration-300 ${
